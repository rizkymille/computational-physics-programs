...
n = length(x);
if length(y) ~= n, error('x dan y harus mempunyai panjang yang sama'); end
if any(diff(x) <= 0), error('x tidak benar-benar naik'); end
...
for i = 1:m %perform interpolations at desired values
    [yns(i),dy(i),d2(i)] = SplineInterp(x, n, a, b, c, d, xx(i));
end
